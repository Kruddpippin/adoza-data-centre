...
import { Camera, X, SwitchCamera, Check, RotateCcw } from "lucide-react";
...
export function WebcamCaptureButton({ label, onCapture, facingMode = "user" }) {
  ...
  // The frame just taken, held here for the candidate to approve before it's ever
  // passed to onCapture — nothing is saved until they confirm it.
  ...
  // Revoke the previous captured frame's object URL whenever it's replaced or cleared,
  // and on unmount — avoids leaking a blob URL per retake.
  useEffect(() => {
    if (!captured) return;
  ...
  const capture = () => {
    ...
    // Hold the frame for review instead of saving immediately — the live camera
    // stream stays open underneath so a retake is instant.
    canvas.toBlob(
      (blob) => {
          ...
          {error ? (
            ...
            <>
              ...
              <div className="relative w-full max-w-xs mx-auto overflow-hidden rounded-lg bg-black" style={{ paddingTop: "133.333%" }}>
                ...
                {/* Circular face guide: dims everything outside it so the candidate can
                    line their face up before capturing. Only shown on the live view. */}
                {!captured && ready && (
                  <div
                ...
                {/* Action buttons on the side of the frame, not below it — reachable with
                    one thumb while holding the phone up to frame a shot. */}
                  ...
                  {captured ? (
                    <>
                      ...
                      <button
                        ...
                        className="flex h-12 w-12 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-lg ring-2 ring-white/80 transition-transform hover:scale-105"
                      >
              ...
              <p className="text-center text-[11px] text-muted-foreground">
                {captured
                  ? "Check the photo, then approve or retake."
                  : switchError || "Position your face inside the circle, then capture."}
              </p>
            </>
          )}
        </div>
